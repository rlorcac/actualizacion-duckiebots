...
class Template(Node):
	def __init__(self,node_name="test", args=None):
		...
		self.args = args
		self.sub = self.create_subscription(Image,"/duckiebot/camera_node/image/raw", self.procesar_img, qos_profile=0) #Cambio de la estructura del subscriber
		self.pubImg = self.create_publisher(Image,"/duckiebot/detecciones",queue_size = 10, qos_profile=0) #cambio de la estructura del publisher
		self.pubPos = self.create_publisher(Point, "/duckiebot/posicionPato", queue_size=10, qos_profile=0) #cambio de la estructura del publisher
	def publicar(self):
		pass
	...
	def procesar_img(self, img):
		...
		self.pubImg.publish(msg) # No estoy seguro que sea asi

def main():
    rclpy.init(args=sys.argv)
    node = Template(node_name="clase_5") # Crea un objeto del tipo Template, cuya definicion se encuentra arriba
