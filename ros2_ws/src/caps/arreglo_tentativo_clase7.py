...
class Template(None):
	def __init__(self,node_name="test", path=None):
		super().__init__(node_name)
		self.path = path
		self.sub = self.create_subscriber(Image,"/duckiebot/camera_node/image/rect", self.procesar_img, qos_profile=0) #Cambio de la estructura del subscrber
		self.pub = self.create_publisher(Image,"/duckiebot/camera_node/image/patos_cv", queue_size = 10, qos_profile=0) #Cambio de la estructura del Publisher
...
def main():
    ...
    node = Template(node_name='clase_7') #creacion y registro del nodo!
